...
export async function GET(req: NextRequest) {
  try {
    ...
    const bookings = await getCustomerBookings(user.maKhachHang!);
    
    // Format and categorize bookings
...
/**
 * Format booking data and categorize by status
 */
function formatAndCategorizeBookings(bookings: any[]): BookingStatus {
  ...
  // Categorize bookings
  const newBookings = formattedBookings.filter(
    booking => booking.trangThai === null
  );
  
  const oldBookings = formattedBookings.filter(
    booking => booking.trangThai === "Check_in" || booking.trangThai === "Check_out"
  );
